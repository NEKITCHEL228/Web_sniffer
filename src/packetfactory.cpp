...
std::shared_ptr<Packet> PacketFactory::createPacket(const unsigned char* data, int length) {
    ...
    if (protocolType == 6) { // TCP
        ...
        packet = std::make_shared<TcpPacket>(srcIP, destIP, length, sp, dp);
    } else if (protocolType == 17) { // UDP
        uint16_t sp = (data[34] << 8) | data[35];
        uint16_t dp = (data[36] << 8) | data[37];
        packet = std::make_shared<UdpPacket>(srcIP, destIP, length, sp, dp);
    } else if (protocolType == 1) { // ICMP
        packet = std::make_shared<IcmpPacket>(srcIP, destIP, length, data[34]);
    }

    if (packet) {
        packet->setRawData(std::vector<uint8_t>(data, data + length));
    }
    return packet;
}
